...
// snapshot.
func routesFromSnapshot(cfgSnap *proxycfg.ConfigSnapshot, _ string, sf supportedProxyFeatures) ([]proto.Message, error) {
	if cfgSnap == nil {
	...
	case structs.ServiceKindConnectProxy:
		return routesFromSnapshotConnectProxy(cfgSnap, sf)
	case structs.ServiceKindIngressGateway:
		return routesFromSnapshotIngressGateway(cfgSnap, sf)
	default:
...
func routesFromSnapshotConnectProxy(cfgSnap *proxycfg.ConfigSnapshot, sf supportedProxyFeatures) ([]proto.Message, error) {
	if cfgSnap == nil {
		return nil, errors.New("nil config given")
	}
...
// "routes" in the snapshot.
func routesFromSnapshotIngressGateway(cfgSnap *proxycfg.ConfigSnapshot, sf supportedProxyFeatures) ([]proto.Message, error) {
	if cfgSnap == nil {
		return nil, errors.New("nil config given")
	}
	...
	for listenerKey, upstreams := range cfgSnap.IngressGateway.Upstreams {
		...
		for _, u := range upstreams {
			...
			virtualHost, err := makeUpstreamRouteForDiscoveryChain(upstreamID, chain, domains, sf)
			if err != nil {
				return nil, err
			}
...
func makeUpstreamRouteForDiscoveryChain(
	...
	case structs.DiscoveryGraphNodeTypeRouter:
		...
		for _, discoveryRoute := range startNode.Routes {
			routeMatch := makeRouteMatchForDiscoveryRoute(discoveryRoute, sf)
...
func makeRouteMatchForDiscoveryRoute(
	discoveryRoute *structs.DiscoveryRoute,
	sf supportedProxyFeatures,
) *envoyroute.RouteMatch {
